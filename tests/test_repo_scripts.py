...
def test_official_and_ingest_runner_scripts_exist():
    ...
    assert (PROJECT_ROOT / 'scripts' / 'run_severity_official_windows.ps1').exists()
...
def test_severity_runner_scripts_call_the_new_pipeline_modules():
    windows_text = (PROJECT_ROOT / 'scripts' / 'run_severity_official_windows.ps1').read_text(encoding='utf-8')
    mac_text = (PROJECT_ROOT / 'scripts' / 'run_severity_official_mac_linux.sh').read_text(encoding='utf-8')
